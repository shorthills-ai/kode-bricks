...
export class Task extends EventEmitter<ClineEvents> {
	...
	public async recursivelyMakeClineRequests(
		...
		if (this.isPaused && provider) {
			...
			if (currentDomain !== this.pausedDomainSlug) {
				await provider.handleDomainSwitch(this.pausedModeSlug)

				await delay(500)
				provider.log(
					`[subtaks] task ${this.taskId}.${this.instanceId} has switched back to '${this.pausedDomainSlug}' from '${currentDomain}'`,
				)
			}
		}
